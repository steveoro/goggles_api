...
RSpec.describe Goggles::MeetingEntriesAPI do
  ...
  describe 'GET /api/v3/meeting_entry/:id' do
    ...
    context 'when requesting a non-existing ID,' do
      before do
        expect(GogglesDb::MeetingEntry.exists?(0)).to be false
        get(api_v3_meeting_entry_path(id: 0), headers: fixture_headers)
      end
  ...
  describe 'PUT /api/v3/meeting_entry/:id' do
    ...
    context 'when requesting a non-existing ID,' do
      before do
        expect(GogglesDb::MeetingEntry.exists?(0)).to be false
        put(api_v3_meeting_entry_path(id: 0), params: expected_changes, headers: crud_headers)
      end
  ...
  describe 'DELETE /api/v3/meeting_entry/:id' do
    ...
    context 'when requesting a non-existing ID,' do
      before do
        expect(GogglesDb::MeetingEntry.exists?(0)).to be false
        delete(api_v3_meeting_entry_path(id: 0), headers: crud_headers)
      end
  ...
  describe 'GET /api/v3/meeting_entries/' do
    ...
    context 'when filtering by a non-existing value,' do
      before do
        ...
        get(api_v3_meeting_entries_path, params: { swimmer_id: 0 }, headers: fixture_headers)
      end
