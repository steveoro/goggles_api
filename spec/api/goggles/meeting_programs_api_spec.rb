...
RSpec.describe Goggles::MeetingProgramsAPI do
  ...
  describe 'GET /api/v3/meeting_program/:id' do
    ...
    context 'when requesting a non-existing ID,' do
      before do
        expect(GogglesDb::MeetingProgram.exists?(0)).to be false
        get(api_v3_meeting_program_path(id: 0), headers: fixture_headers)
      end
  ...
  describe 'PUT /api/v3/meeting_program/:id' do
    ...
    context 'when requesting a non-existing ID,' do
      before do
        expect(GogglesDb::MeetingProgram.exists?(0)).to be false
        put(api_v3_meeting_program_path(id: 0), params: expected_changes, headers: admin_headers)
      end
  ...
  describe 'POST /api/v3/meeting_program' do
    ...
    context 'when using missing or invalid parameters,' do
      before do
        ...
        expect(GogglesDb::GenderType.exists?(0)).to be false
  ...
  describe 'DELETE /api/v3/meeting_program/:id' do
    ...
    context 'when requesting a non-existing ID,' do
      before do
        expect(GogglesDb::MeetingProgram.exists?(0)).to be false
        delete(api_v3_meeting_program_path(id: 0), headers: admin_headers)
      end
  ...
  describe 'GET /api/v3/meeting_programs/' do
    ...
    context 'when filtering by a non-existing value,' do
      before do
        ...
        get(api_v3_meeting_programs_path, params: { meeting_id: 0 }, headers: fixture_headers)
      end
