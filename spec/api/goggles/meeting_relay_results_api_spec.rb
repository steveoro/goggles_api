...
RSpec.describe Goggles::MeetingRelayResultsAPI do
  ...
  describe 'GET /api/v3/meeting_relay_result/:id' do
    ...
    context 'when requesting a non-existing ID,' do
      before do
        expect(GogglesDb::MeetingRelayResult.exists?(0)).to be false
        get(api_v3_meeting_relay_result_path(id: 0), headers: fixture_headers)
      end
  ...
  describe 'PUT /api/v3/meeting_relay_result/:id' do
    ...
    context 'when requesting a non-existing ID,' do
      before do
        expect(GogglesDb::MeetingRelayResult.exists?(0)).to be false
        put(api_v3_meeting_relay_result_path(id: 0), params: expected_changes, headers: crud_headers)
      end
  ...
  describe 'DELETE /api/v3/meeting_relay_result/:id' do
    ...
    context 'when requesting a non-existing ID,' do
      before do
        expect(GogglesDb::MeetingRelayResult.exists?(0)).to be false
        delete(api_v3_meeting_relay_result_path(id: 0), headers: crud_headers)
      end
  ...
  describe 'GET /api/v3/meeting_relay_results/' do
    ...
    context 'when filtering by a non-existing value,' do
      before do
        ...
        get(api_v3_meeting_relay_results_path, params: { team_id: 0 }, headers: fixture_headers)
      end
